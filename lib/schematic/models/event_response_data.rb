  ...
  class EventResponseData
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Schematic::EventResponseData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Schematic::EventResponseData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'enriched_at')
        self.enriched_at = attributes[:'enriched_at']
      end
    ...
    def hash
      [api_key, body, captured_at, company_id, enriched_at, environment_id, feature_id, id, loaded_at, processed_at, processing_status, sent_at, subtype, type, updated_at, user_id].hash
    end
