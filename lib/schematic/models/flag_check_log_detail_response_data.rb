...
module Schematic
  class FlagCheckLogDetailResponseData
    ...
    attr_accessor :req_company

    attr_accessor :req_user
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Schematic::FlagCheckLogDetailResponseData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Schematic::FlagCheckLogDetailResponseData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'req_company')
        self.req_company = attributes[:'req_company']
      end

      if attributes.key?(:'req_user')
        self.req_user = attributes[:'req_user']
      end
    ...
    def hash
      [check_status, company, company_id, created_at, environment, environment_id, error, flag, flag_id, flag_key, id, reason, req_company, req_user, rule, rule_id, updated_at, user, user_id, value].hash
    end
