...
module Schematic
  class CreateOrUpdateFlagRequestBody
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Schematic::CreateOrUpdateFlagRequestBody` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Schematic::CreateOrUpdateFlagRequestBody`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'flag_type')
        self.flag_type = attributes[:'flag_type']
      end
    ...
    def list_invalid_properties
      ...
      if @flag_type.nil?
        invalid_properties.push('invalid value for "flag_type", flag_type cannot be nil.')
      end
    ...
    def hash
      [default_value, description, feature_id, flag_type, id, key, name, rules, skip_webhooks].hash
    end
