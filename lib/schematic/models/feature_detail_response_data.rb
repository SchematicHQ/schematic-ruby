...
module Schematic
  ...
  class FeatureDetailResponseData
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Schematic::FeatureDetailResponseData` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Schematic::FeatureDetailResponseData`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'event_summary')
        self.event_summary = attributes[:'event_summary']
      end
      ...
      if attributes.key?(:'lifecycle_phase')
        self.lifecycle_phase = attributes[:'lifecycle_phase']
      end
    ...
    def hash
      [created_at, description, event_subtype, event_summary, feature_type, flags, id, lifecycle_phase, name, trait_id, updated_at].hash
    end
